...
var _DEFAULT_RESOURCE = {
    ...
    'E' : {
        ...
        5: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday',
            'Saturday']
    },
    'a' : {
        1: ['a', 'p'],
...
function _pad (n, size) {
    size = size > _ZEROS.length ? _ZEROS.length : size;
    n = n + '';
    return _ZEROS.substring(0, size - n.length) + n;
}
...
var _LOCAL_TZO = _getTimezoneOffset(new Date());
 ...
 * The formatter class
 */
function TimeFormatter(resource){

    this.localTimezoneOffset = _LOCAL_TZO;
    this.res = resource || _DEFAULT_RESOURCE;
    this.resKeys = {};
    var k;
 ...
 */
module.exports = timeformater = function(resource){
    return new TimeFormatter(resource);
}
