...
function TimeFormatter(lang){
    ...
    lang = lang || 'en';
    this.res     = TimeFormatter.getResource(lang);
    this.resKeys = this.res['__keys__'];
     ...
     */
    this.getResourceOf = function(val, symbol, len){
        var r = this.res[symbol];
        var rk = this.resKeys[symbol];
 ...
 */
TimeFormatter.addResource = function(lang, arr){
    TimeFormatter.resources[lang] = arr;
    var keys = [];
    for (var s in arr){ // s for symbol
        var keysOfSymbol = [];
        for (var k in arr[s]){
            keysOfSymbol.push(k);
        }
        keys[s] = keysOfSymbol.sort();
    }
    TimeFormatter.resources[lang]['__keys__'] = keys;
};

/**
 *
 */
TimeFormatter.getResource = function(lang){
    if(!TimeFormatter.resources[lang] && typeof module != 'undefined'){
        var path = require("path").join(__dirname, '..', 'lang', lang);
        TimeFormatter.addResource(lang, require(path));
    }
    return TimeFormatter.resources[lang];
};
